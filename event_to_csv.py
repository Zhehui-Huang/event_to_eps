...
def extract(dpath, subpath):
    scalar_accumulators = [EventAccumulator(str(dpath / subpath)).Reload().scalars]
...
def write_csv(dpath, subpath, key, fname, aggregations, steps, aggregation_ops):
    ...
    df.to_csv(path / file_name, sep=';', header=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--subpaths", type=param_list, help="subpath sturctures", default=['event'])
    ...
    #subpaths = os.path.join(path, 'test_param_xy', 'test')
    subpaths = os.path.join(path, args.subpaths[0])
